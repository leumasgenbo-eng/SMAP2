  ...
  const handleSharePDF = async () => {
    ...
    try {
        ...
        if (navigator.canShare && navigator.canShare({ files: [file] })) {
            await navigator.share({ files: [file], title: 'Early Childhood Master Sheet', text: 'Attached is the master broad sheet.' });
        } else {
    ...
    <div className="bg-white p-4 print:p-0 min-h-screen font-sans text-sm">
       <div className="flex justify-end mb-4 no-print">
          <button onClick={handleSharePDF} disabled={isGenerating} className="bg-green-600 hover:bg-green-700 text-white px-4 py-2 rounded shadow font-bold flex items-center gap-2">
            {isGenerating ? 'Generating...' : <><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M4 12v8a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2v-8"></path><polyline points="16 6 12 2 8 6"></polyline><line x1="12" y1="2" x2="12" y2="15"></line></svg>Share PDF</>}
          </button>
       </div>
      ...
      <div id="daycare-master-print-area">
        ...
        <div className="overflow-x-auto mb-8 border border-gray-300 shadow-sm rounded">
            ...
            <thead>
                ...
                {coreSubjects.map(sub => (<th key={sub} className="border border-blue-800 p-2 min-w-[50px] text-center bg-blue-800 align-bottom" title={sub}><div style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }} className="h-40 flex items-center justify-center mx-auto text-xs leading-none whitespace-nowrap">{getShortName(sub)}</div></th>))}
                {indicators.map(ind => (<th key={ind} className="border border-blue-800 p-2 min-w-[35px] text-center bg-blue-700 align-bottom" title={ind}><div style={{ writingMode: 'vertical-rl', transform: 'rotate(180deg)' }} className="h-40 flex items-center justify-center mx-auto text-[10px] leading-none whitespace-nowrap">{getShortName(ind)}</div></th>))}
            ...
            <tbody>
                ...
                <tr key={student.id} className="hover:bg-blue-50 border-b border-gray-200">
                    ...
                    return (<td key={sub} className="border-r p-2 text-center"><div className="flex flex-col items-center"><span className="font-bold">{score}</span><span className={`text-[10px] ${colorClass}`}>{grade}</span></div></td>);
                    })}
                    {indicators.map(ind => {
                        ...
                        return (<td key={ind} className={`border-r p-1 text-center font-bold text-xs ${bgClass}`}>{rating}</td>);
                    })}
                    <td className="border-r p-2 text-center text-xs">{student.promotedTo || '-'}</td>
                    <td className="border-r p-2 text-center text-xs">{student.attendance || 0}/{settings.attendanceTotal}</td>
                </tr>
                ))}
            </tbody>
            </table>
